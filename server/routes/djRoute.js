...
router.post('/remove-expired-bookings', authMiddleware, async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: 'Przedawnione rezerwacje dla danego DJ-a zostały usunięte, a powiadomienia zostały wysłane do użytkowników.',
    });
  } catch (error) {
    console.error(error);
    res.status(500).json({
      success: false,
      message: 'Wystąpił błąd podczas usuwania przedawnionych rezerwacji i wysyłania powiadomień.',
    });
  }
});
