...
router.post('/register', async (req, res) => {
  try {
    ...
    await newuser.save();
    res.status(200).send({ message: 'Zarejestrowano pomyślnie', success: true });
  } catch (error) {
    console.log(error);
    res.status(500).send({ message: 'Błąd rejestracji', success: false, error });
  }
});
...
router.post('/reset-password', async (req, res) => {
  try {
    ...
    user.save();
    res.status(200).send({ message: 'Pomyślnie zmieniono hasło', success: true });
  } catch (error) {
    console.log(error);
    res.status(500).send({ message: 'Błąd zmiany hasła', success: false, error });
  }
});
...
router.post('/book-dj', authMiddleware, async (req, res) => {
  try {
    ...
    djAsUser.unseenNotifications.push({
      ...
      message: `Nowa rezerwacja została utworzona przez: ${user.name}`,
